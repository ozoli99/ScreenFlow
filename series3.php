<?php
    ...
    if (isset($_GET["watched"])) {
        foreach ($users as $key => $user) {
            if ($user["login"]) {
                $users[$key]["watched"]["series3"] = $_GET["watched"] - 1;
            }
        }
    }
    ...
    if (isset($_GET["unwatched"])) {
        foreach ($users as $key => $user) {
            if ($user["login"]) {
                $users[$key]["watched"]["series3"] = $_GET["unwatched"];
            }
        }
    }
...
<html lang="en">
    <head>
        ...
        <title><?= $series3["title"] ?> - ScreenFlow</title>
    </head>
    <body id="page-top">
        <!-- Navigation-->
            ...
            <div class="container px-5">
                ...
                <div class="collapse navbar-collapse" id="navbarResponsive">
                    <ul class="navbar-nav ms-auto">
                        ...
                        <?php if($has_user): ?>
                            <li class="nav-item"><a class="nav-link" href="./series3.php?logout=true">Log Out</a></li>
        ...
        <!-- Header-->
        <header class="masthead text-center text-white bg-image" style="background-image: url('Media/TheWitcherBG.jpg');">
            <div class="masthead-content">
                <div class="container px-5">
                    <h1 class="masthead-heading mb-0"><?= $series3["title"] ?></h1>
                    <h2 class="lead mb-0 mx-5"><?= $series3["plot"] ?></h2>
        ...
        <!-- Content section-->
        <?php $i = 1; ?>
        <?php foreach($series3["episodes"] as $episode): ?>
            <section id="scroll" class="bg-dark text-white py-5">
                <div class="container px-0">
                    <div class="row gx-5 align-items-center">
                        <?php if($has_user): ?>
                            <?php if($i <= $active_user["watched"]["series3"]): ?>
                                <?php if($i % 2 == 1): ?>
                                    ...
                                    <div class="col-lg-7 order-lg-1">
                                        <div class="p-1 py-5">
                                            <h2 class="display-6 text-white-50">
                                                <?= $episode[$i]["title"]?>
                                                <a href="./series3.php?unwatched=<?= $i ?>">
